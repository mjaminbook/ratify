...
def add_answer(request):
	questionId = int(request.POST.get("question_id"))
	answerText = request.POST.get("answer_text")
	question = Question.objects.get(pk=questionId)
	newAnswer = Answer(question=question, answer_text=answerText, num_votes=1) #Should we assume someone adding an answer is a vote Yes i think we should
	newAnswer.save()
